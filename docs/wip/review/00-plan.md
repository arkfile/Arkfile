...
| F — Frontend/WASM/Supply/Ops | `06-frontend-supply-ops.md` | **In progress** | 2026-05-11 | | Session started 2026-05-11. Scope: frontend (HTML/CSS + all `client/static/js/src/**` not covered by A-D), WASM artifact integrity (`opaque.js`, any Argon2id WASM), service worker, supply chain (`package.json`, lockfile, `go.mod`/`go.sum`, `.gitmodules`, `config/dependency-hashes.json`), CLI binary build (`go build` flags as written in current scripts — `-trimpath`, `-buildid=`, `-ldflags='-s -w'`, `CGO_LDFLAGS` static-linking, RPATH/RUNPATH, signing/provenance), Caddy/TLS/headers (`Caddyfile*` — CSP, HSTS, `X-Forwarded-For`, CORS, TLS posture, deSEC integration as written), systemd hardening (all four `.service` units), deployment-script content (file modes on `/opt/arkfile/etc/**`, secret handling on disk, `set -euo pipefail`, sudo escalation as written), public monitoring endpoints. Operator-side review limited to "what's in the script today" — no attacker-via-operator threat modeling (deferred to Slice G). |
