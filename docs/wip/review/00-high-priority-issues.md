...
| B | — | A-04, F-08, A-05, F-07 | **Done 2026-05-15** | Server-side `__Host-*` HttpOnly cookies issued at login/TOTP-verify/refresh/logout. `CookieTokenMiddleware` + `CSRFMiddleware` wired globally. `GET /api/auth/me` for browser identity lookup. `handlers/cookies.go`, `handlers/auth.go`, `handlers/middleware.go`, `handlers/route_config.go` updated. Frontend: `utils/auth.ts`, `login.ts`, `totp.ts`, `totp-setup.ts`, `contact-info.ts`, `billing.ts`, `share-list.ts`, `share.ts`, `download.ts`, `upload.ts`, `streaming-download.ts` — all `Authorization: Bearer` / `getToken()` / `localStorage` auth replaced with cookie+CSRF model. `window.totpLoginData` replaced by module-private `_pendingTOTPFlowData`. `setTokens()` and `getToken()` stubs retained as no-ops for call-site compatibility. 8 new Go middleware unit tests for `CookieTokenMiddleware` + `CSRFMiddleware` (all pass). TS `auth-manager.test.ts` rewritten for cookie model. All 10 Go packages green. Remaining before final validation close: run `dev-reset.sh` + `e2e-test.sh` + `e2e-playwright.sh`; then commit. Beta impact: YELLOW (forced re-login + hard-refresh). |
