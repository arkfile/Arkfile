...
All tag parsing, validation, encryption, decryption, searching, and filtering happen on the client. The server stores only opaque `encrypted_tags` and `tags_nonce` (both present or both absent/empty as a pair). There are no blind indexes, no plaintext tag columns, and no server-side tag query parameters. Because the server cannot decrypt the list, it cannot verify how many tags a client changed or prevent a custom client from replacing or emptying the whole list. Official clients trim whitespace around commas, reject spaces inside a tag, and surface over-limit input as `N tags maximum` (not a whitespace error). Filter logic runs against already-decrypted in-memory metadata after the owner list is loaded. At large vault sizes (for example 10,000 files each with five max-length tags), matching itself is expected to be milliseconds; the dominant costs remain full-list fetch, Account Key metadata decrypt, and (on the web) DOM rendering -- the same costs owners already pay for encrypted filenames. Tag reuse rates do not reduce decrypt work because each file still carries its own ciphertext blob.

Post-upload editing uses an authenticated owner metadata update path. Tags may also be supplied at upload time. After upload, owners edit one file at a time: Add accepts one tag or a comma-separated list (merged in memory, one PUT); Remove and Replace stay single-tag actions. There is no "delete all tags" / clear-all action and no multi-file batch retag in v1. The wire still carries one ciphertext for the full tag list. Shares, public share metadata, and share envelopes stay unchanged. Owner `.arkbackup` export must carry the encrypted tags fields in the version-2 JSON header so offline decrypt can restore organization (for example tags that encode former folder paths or topics); admin export continues to be undecryptable without the owner's password.
...
| Post-upload edit model | One file at a time; Add accepts one tag or comma-separated many (one PUT); Remove and Replace remain one tag (or one pair) per action; no clear-all / delete-all-tags |
| Server enforcement boundary | Official clients enforce no clear-all control and the Add/Remove/Replace UX above; the privacy-preserving server cannot inspect or compare plaintext lists, so it validates only ownership, field pairing, encoded size, and revision |
| Upload-time tags | A single upload may assign several initial tags through the canonical comma-separated input (same trim rules as Edit Add) |
| Limit error wording | Over-limit file tag lists surface as `N tags maximum` (default `5 tags maximum`); spaces around commas are never reported as whitespace errors |
...
- No dedicated clear-all API, request flag, or CLI/UI control that wipes every tag in one user gesture. Clients expose add (one or many), remove one, and replace one.
...
- File detail / actions: each decrypted file row has an `Edit tags` button/link that opens a modal for that file. Add accepts one tag or comma-separated many; remove/replace stay single-tag. Decrypt current tags, mutate in memory, re-encrypt, PUT with the revision that was read. Replace is a first-class single action, not a remove followed by an add. No "clear all tags" button. No multi-select batch retag in v1.
...
- Group mutation under `arkfile-client tags add` (one tag or comma-separated list), `arkfile-client tags remove`, and `arkfile-client tags replace`; each requires `--file-id`, decrypts current tags when needed, mutates, re-encrypts, and calls the update API with the revision that was read. There is no `tags clear` command.
