...
#### `rotate-opaque-keys.sh`
**Purpose**: OPAQUE server key rotation runbook (delegates to `arkfile-admin rotate-opaque-keys rotate`)  
**Usage**: `bash scripts/maintenance/rotate-opaque-keys.sh`  
**Requires**: Prior `arkfile-admin login`; no service restart (keys reload in-process)
