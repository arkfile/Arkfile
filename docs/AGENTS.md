...
- **Which script to use after code changes:** Use whichever script matches your current workflow. If you are in a dev iteration loop running e2e tests, use `dev-reset.sh`. If you are running a local deployment, use `local-deploy.sh`. Both handle full recompilation and redeployment. Never use one when you mean the other.
