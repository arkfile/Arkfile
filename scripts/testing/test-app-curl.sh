...
$metadata_decrypt_output"
    ...
    info "[OK] Metadata fetched once from dedicated /meta endpoint"
    info "[OK] Chunks downloaded without redundant metadata headers"  
    info "[OK] Efficient API flow reduces server load and improves performance"
    ...
    if [ "$file_hash" = "$final_e2e_hash" ]; then
        ...
        info "  [OK] Generate original file"
        info "  [OK] Encrypt content + metadata"
        info "  [OK] Upload encrypted content + encrypted metadata"
        info "  [OK] Store and retrieve encrypted metadata"
        info "  [OK] Decrypt metadata (filenames, hashes)"
        info "  [OK] Download encrypted content in chunks"
        info "  [OK] Decrypt content"
        info "  [OK] Verify perfect integrity through entire cycle"
