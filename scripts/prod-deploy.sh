...
STORAGE_PROVIDER_1_ID=generic-s3:arkfile
STORAGE_1_ENDPOINT=http://localhost:9332
STORAGE_1_ACCESS_KEY=arkfile
STORAGE_1_SECRET_KEY=${s3_password}
STORAGE_1_BUCKET=arkfile
...
start_and_verify_services() {
    ...
    echo "user = \"arkfile-db:${RQLITE_PASSWORD}\"" > "$tmp_curl_config"
...
if [ "$EXISTING_DEPLOYMENT" = "true" ]; then
    ...
    echo
    echo "  To update an existing deployment, run: sudo bash scripts/prod-update.sh"
