...
if grep -q "^ARKFILE_MASTER_KEY=" "$SECRETS_FILE"; then
    ...
    # Append to secrets.env. This value can be rotated later without downtime to
    # data: 'arkfile-admin rotate-envelope-master' (or
    # scripts/maintenance/rotate-envelope-master.sh) re-wraps every system_keys
    # row under a fresh master and rewrites this ARKFILE_MASTER_KEY line in place.
    echo "" >> "$SECRETS_FILE"
    echo "# Master Key for Envelope Encryption (generated by 03-setup-master-key.sh)" >> "$SECRETS_FILE"
    echo "# Rotatable via: arkfile-admin rotate-envelope-master prepare|apply" >> "$SECRETS_FILE"
