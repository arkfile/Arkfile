#!/bin/bash
# Shared VPS first-time deploy body for prod-deploy.sh and test-deploy.sh.
# Source this AFTER setting profile variables and sourcing build-config.sh + deploy-common.sh.
#
# Required profile variables (set by the thin wrapper):
#   VERSION_PREFIX          prod | test
#   RQLITE_USERNAME         arkfile-db | test-user
#   SEAWEED_ACCESS_KEY      arkfile | arkfile-test
#   SEAWEED_BUCKET          arkfile | arkfile-test
#   SEAWEED_PROVIDER_ID     generic-s3:arkfile | generic-s3:arkfile-test
#   SEAWEED_IDENTITY_NAME   arkfile (seaweedfs-s3.json identity name; same for prod and test)
#   CADDYFILE_TEMPLATE      Caddyfile.prod | Caddyfile.test
#   DEPLOY_KIND_LABEL       Production | Test
#   DEPLOY_SCRIPT_NAME      prod-deploy.sh | test-deploy.sh
#   UPDATE_SCRIPT_HINT      scripts/prod-update.sh | scripts/test-update.sh
#   DOMAIN_EXAMPLE          example domain for help text
#   BANNER_TITLE            banner string
#   COMPLETE_TITLE          completion banner
#   INSTANCE_BLURB          one-line blurb after complete
#
# Also expects: SCRIPT_DIR, ARKFILE_DIR, ARKFILE_USER, ARKFILE_GROUP, colors, ORIGINAL_*.

# Runtime defaults (profile vars must already be set by the wrapper)
DOMAIN="${DOMAIN:-}"
...
FORCE_REBUILD_RQLITE="${FORCE_REBUILD_RQLITE:-false}"
EXTERNAL_FIREWALL_CONFIRMED="${EXTERNAL_FIREWALL_CONFIRMED:-false}"
...
build_application() {
    ...
    run_application_build "${VERSION_PREFIX}-$(date +%Y%m%d-%H%M%S)" --production
...
# ${DEPLOY_KIND_LABEL} Deployment Configuration
# Generated: $(date)
# Domain: ${DOMAIN}
HOST=127.0.0.1
...
RQLITE_USERNAME=${RQLITE_USERNAME}
...
STORAGE_PROVIDER_1_ID=${SEAWEED_PROVIDER_ID}
STORAGE_1_ENDPOINT=http://localhost:9332
STORAGE_1_ACCESS_KEY=${SEAWEED_ACCESS_KEY}
STORAGE_1_SECRET_KEY=${s3_password}
STORAGE_1_BUCKET=${SEAWEED_BUCKET}
...
STORAGE_PROVIDER_1_ID=${STORAGE_BACKEND}:${S3_BUCKET}
STORAGE_1_REGION=${S3_REGION}
STORAGE_1_ACCESS_KEY=${S3_ACCESS_KEY}
STORAGE_1_SECRET_KEY=${S3_SECRET_KEY}
STORAGE_1_BUCKET=${S3_BUCKET}
...
STORAGE_PROVIDER_1_ID=backblaze:${BACKBLAZE_BUCKET_NAME}
STORAGE_1_ENDPOINT=${BACKBLAZE_ENDPOINT}
STORAGE_1_ACCESS_KEY=${BACKBLAZE_KEY_ID}
STORAGE_1_SECRET_KEY=${BACKBLAZE_APPLICATION_KEY}
STORAGE_1_BUCKET=${BACKBLAZE_BUCKET_NAME}
STORAGE_1_REGION=us-east-1
...
EOF2
        ...
        cloudflare-r2)
            cat >> "$ARKFILE_DIR/etc/secrets.env" <<EOF2
# Storage Configuration
STORAGE_PROVIDER_1=cloudflare-r2
STORAGE_PROVIDER_1_ID=cloudflare-r2:${CLOUDFLARE_BUCKET_NAME}
STORAGE_1_ENDPOINT=${CLOUDFLARE_ENDPOINT}
STORAGE_1_ACCESS_KEY=${CLOUDFLARE_ACCESS_KEY_ID}
STORAGE_1_SECRET_KEY=${CLOUDFLARE_SECRET_ACCESS_KEY}
STORAGE_1_BUCKET=${CLOUDFLARE_BUCKET_NAME}
STORAGE_1_REGION=us-east-1
...
# Storage Configuration
STORAGE_PROVIDER_1=generic-s3
STORAGE_PROVIDER_1_ID=generic-s3:${S3_BUCKET}
STORAGE_1_ENDPOINT=${S3_ENDPOINT}
STORAGE_1_REGION=${S3_REGION}
STORAGE_1_ACCESS_KEY=${S3_ACCESS_KEY}
STORAGE_1_SECRET_KEY=${S3_SECRET_KEY}
STORAGE_1_BUCKET=${S3_BUCKET}
STORAGE_1_FORCE_PATH_STYLE=${S3_FORCE_PATH_STYLE}
...
# Billing / storage credits (production cadence)
...
EOF2
    ...
    # Use a systemd drop-in override for EnvironmentFile instead of sed-patching the service file
    mkdir -p /etc/systemd/system/caddy.service.d
    cat > /etc/systemd/system/caddy.service.d/env.conf <<EOF2
[Service]
EnvironmentFile=/var/lib/caddy/caddy-env
EOF2
    systemctl daemon-reload
    print_status "INFO" "Installed caddy.service drop-in for EnvironmentFile at /var/lib/caddy/caddy-env"
...
start_and_verify_services() {
    ...
    echo "user = \"${RQLITE_USERNAME}:${RQLITE_PASSWORD}\"" > "$tmp_curl_config"
...
if [ -z "$DOMAIN" ] || [ -z "$ADMIN_USERNAME" ]; then
    print_status "ERROR" "--domain and --admin-username are required"
    show_help
    exit 1
fi
...
if [ "$EXISTING_DEPLOYMENT" = "true" ]; then
    ...
    echo
    echo "  To update an existing deployment, run: sudo bash ${UPDATE_SCRIPT_HINT}"
