...
func TestGetPendingUsers_Success_Admin(t *testing.T) {
	...
	adminUserRows := sqlmock.NewRows([]string{"id", "email", "password", "created_at", "total_storage_bytes", "storage_limit_bytes", "is_approved", "approved_by", "approved_at", "is_admin"}).
		AddRow(1, adminEmail, "hashedpassword", time.Now(), int64(0), models.DefaultStorageLimit, true, sql.NullString{}, sql.NullTime{}, true) // Admin user
	mockDB.ExpectQuery(`
...
func TestDeleteUser_Forbidden_NonAdmin(t *testing.T) {
	...
	mockDB.ExpectQuery(`
		...
		sqlmock.NewRows([]string{"id", "email", "password", "created_at", "total_storage_bytes", "storage_limit_bytes", "is_approved", "approved_by", "approved_at", "is_admin"}).
			AddRow(2, nonAdminEmail, "nonadminpass", time.Now(), int64(0), models.DefaultStorageLimit, true, sql.NullString{}, sql.NullTime{}, false))
...
func TestUpdateUser_Error_Forbidden_NonAdmin(t *testing.T) {
	...
	mockDB.ExpectQuery(`
		...
		FROM users WHERE email = ?`).WithArgs(nonAdminEmail).WillReturnRows(
		sqlmock.NewRows([]string{"id", "email", "password", "created_at", "total_storage_bytes", "storage_limit_bytes", "is_approved", "approved_by", "approved_at", "is_admin"}).
			AddRow(2, nonAdminEmail, "userpass", time.Now(), int64(0), models.DefaultStorageLimit, true, sql.NullString{}, sql.NullTime{}, false))
...
func TestUpdateUser_Error_InvalidJSON(t *testing.T) {
	...
	require.True(t, ok)
	assert.Equal(t, http.StatusBadRequest, httpErr.Code)
	assert.Equal(t, "Invalid request format", httpErr.Message)
...
func TestUpdateUser_RevokeAccess_Forbidden_NonAdmin(t *testing.T) {
	...
	mockDB.ExpectQuery(`
		...
		FROM users WHERE email = ?`).WithArgs(nonAdminEmail).WillReturnRows(
		sqlmock.NewRows([]string{"id", "email", "password", "created_at", "total_storage_bytes", "storage_limit_bytes", "is_approved", "approved_by", "approved_at", "is_admin"}).
			AddRow(1, nonAdminEmail, "userpass", time.Now(), int64(0), models.DefaultStorageLimit, true, sql.NullString{}, sql.NullTime{}, false))
...
func TestUpdateUser_RevokeAccess_SimulateTokenDeleteError(t *testing.T) {
	...
	c, rec, mockDB, _ := setupTestEnv(t, http.MethodPut, "/admin/users/:email/update", bytes.NewReader(jsonBody)) // Use UpdateUser endpoint
	c.SetParamNames("email")
	c.SetParamValues(targetUserEmail)
	...
	// If token deletion were part of UpdateUser (it's not, but simulating the original test's intent if it were a more comprehensive revoke)
	// deleteTokensSQL := `DELETE FROM refresh_tokens WHERE user_email = ?`
	// mockDB.ExpectExec(deleteTokensSQL).WithArgs(targetUserEmail).WillReturnError(fmt.Errorf("DB error deleting tokens")) // Simulate error here

	logAdminActionSQL := `INSERT INTO admin_logs (admin_email, action, target_user_email, details) VALUES (?, ?, ?, ?)`
	mockDB.ExpectExec(logAdminActionSQL).
		WithArgs(adminEmail, "revoke approval", targetUserEmail, ""). // Action matches UpdateUser when isApproved:false
		WillReturnResult(sqlmock.NewResult(1, 1))

	mockDB.ExpectCommit() // Assuming the update itself commits

	err := UpdateUser(c)
	require.NoError(t, err) // UpdateUser itself should succeed for the DB part it controls.
	assert.Equal(t, http.StatusOK, rec.Code)

	var resp map[string]string
	err = json.Unmarshal(rec.Body.Bytes(), &resp)
	require.NoError(t, err)
	assert.Equal(t, "User updated successfully", resp["message"]) // UpdateUser's message

	// If UpdateUser *did* try to delete tokens and it failed, we'd check logBuf.
	// Since it *doesn't*, this log check wouldn't apply for a pure UpdateUser test of its current scope.
...
func TestListUsers_Success_Admin(t *testing.T) {
	...
	assert.Equal(t, "10.00 GB", firstUser["storageLimitReadable"])
	assert.Equal(t, "1.00 GB", firstUser["totalStorageReadable"])
	assert.InDelta(t, 10.0, firstUser["usagePercent"], 0.01)
...
func TestGetPendingUsers_Forbidden_NonAdmin(t *testing.T) {
	...
	nonAdminUserRows := sqlmock.NewRows([]string{"id", "email", "password", "created_at", "total_storage_bytes", "storage_limit_bytes", "is_approved", "approved_by", "approved_at", "is_admin"}).
		AddRow(1, nonAdminEmail, "hashedpassword", time.Now(), int64(0), models.DefaultStorageLimit, true, sql.NullString{}, sql.NullTime{}, false)
	mockDB.ExpectQuery(`
		SELECT id, email, password, created_at,
		       total_storage_bytes, storage_limit_bytes,
		       is_approved, approved_by, approved_at, is_admin
...
func TestGetPendingUsers_GetPendingError(t *testing.T) {
	...
	adminUserRows := sqlmock.NewRows([]string{"id", "email", "password", "created_at", "total_storage_bytes", "storage_limit_bytes", "is_approved", "approved_by", "approved_at", "is_admin"}).
		AddRow(1, adminEmail, "hashedpassword", time.Now(), int64(0), models.DefaultStorageLimit, true, sql.NullString{}, sql.NullTime{}, true)
	mockDB.ExpectQuery(`
		SELECT id, email, password, created_at,
		       total_storage_bytes, storage_limit_bytes,
		       is_approved, approved_by, approved_at, is_admin
