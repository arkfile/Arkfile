...
func validateSocketSecurity(socketPath string, expectedUID int) error {
	...
	// Check ownership (POSIX_XUCRED on FreeBSD)
...
// using LOCAL_PEERCRED on FreeBSD, ensuring only the owner of the agent can make requests.
func isPeerAuthorized(conn net.Conn) (bool, error) {
	...
	// unix.GetsockoptXucred is standard on FreeBSD and returns *unix.Xucred
